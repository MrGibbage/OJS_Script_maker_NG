...
# Double-click to run.
#
# To create mac os executables, first git push the latest update
# Open the mac os virtual machine in proxmox
# ssh into the vm
# cd /Users/skip/Documents/OJS_Script_maker_NG
# git pull
# activate the python virtual machine (will see venv in the command prompt)
# source .venv/bin/activate
# cd 2025
# venv/bin/pyinstaller -F script-maker-mac-win.py -n script_maker-mac
# When complete, the mac os execuatble will be saved in the dist directory
# cp dist/script_maker-mac .
# cd /Users/skip/Documents/OJS_Script_maker_NG
# git add *
# git commit -m "Updated mac os executable"
# git push origin main:main
# For the password, use the personal access token
#
# Note that the mac os executable will take a minute or two to run.
# This seems normal, and is maybe a lot faster on real mac hardware
...
try:
    ...
    print(Fore.RED + 
        f"Fatal error starting generator: {e}"
    )
    input("Press enter to quit...")
    sys.exit(1)
...
try:
    ...
    for tourn_filename in directory_list:
        ...
        for i in reversed(range(awardCounts["Robot Game"])):
            try:
                teamNum = dfRankings.loc[
                    dfRankings["Robot Game Rank"] == i + 1, "Team Number"
                ].values[0]
                teamName = dfRankings.loc[
                    dfRankings["Robot Game Rank"] == i + 1, "Team Name"
                ].values[0]
                score = int(
                    dfRankings.loc[
                        dfRankings["Robot Game Rank"] == i + 1, "Max Robot Game Score"
                    ].values[0]
                )
            except:
                print(Fore.RED + 
        ...
        for award in awards:
            ...
            for i in reversed(range(awardCounts[award])):
                ...
                try:
                    teamNum = dfRankings.loc[
                        dfRankings["Award"] == thisAward, "Team Number"
                    ].values[0]
                    teamName = dfRankings.loc[
        ...
        allowedAdvancingCount[div - 1] = dfMeta.loc[
            dfMeta["Key"] == "Advancing", "Value"
        ].values[0]
        advancingHtml[div] = ""
        ...
        judgesAwardDf[div] = dfRankings[
            (dfRankings["Award"].str.startswith("Judges", na=False))
        ]
        ...
        if divAltAdvancing == 0:
            print(Fore.YELLOW + 
                f"You have not selected one alternative advancing div {div} team.\nThis is not an error and you may continue building the script."
            )
            try:
                input("Press enter to continue. Press ctrl-c to quit...")
            except:
        ...
        if divAltAdvancing > 1:
            print(Fore.YELLOW + 
                f"You have selected more than one alternative advancing div {div} team.\nThis is not an error and you may continue building the script."
            )
            try:
                input("Press enter to continue. Press ctrl-c to quit...")
            except:
