...
# .venv\Scripts\pyinstaller.exe -F 2024\script_maker_path.py
...
if not os.path.exists("file_list.txt"):
    print(Fore.RED + 
        f"Fatal error. The file_list.txt file is missing in the tournament directory"
    )
...
with open("file_list.txt", "r") as file:
    for line in file:
        ...
        if not os.path.exists(line):
            print(Fore.RED + 
                f"Fatal error. The {line} file is missing in the tournament directory"
            )
            input("Press enter to quit...")
            sys.exit(1)

if getattr(sys, 'frozen', False):  # Check if the script is frozen (PyInstaller EXE)
    # Use sys._MEIPASS for the temporary folder where PyInstaller extracts files
    application_path = os.path.dirname(sys.executable)
else:
    # Use __file__ for the script's directory during development
    application_path = os.path.dirname(os.path.abspath(__file__))

try:
    templateLoader = FileSystemLoader(application_path)
    templateEnv = Environment(loader=templateLoader)
except Exception as e:
    print(Fore.RED + 
        f"Fatal error. Could not load the jinja template loader.\nThe error was {e}"
    )
    input("Press enter to quit...")
    sys.exit(1)
...
for tourn_filename in directory_list:
    ...
    m0 = re.search(regex, tourn_filename)
...
# print(out_text)
with open(
    dfMeta.loc[dfMeta["Key"] == "Completed Script File", "Value"].values[0], "w"
) as fh:
    fh.write(out_text)
