...
TEMPLATE_FILE = "script_template_emcee.html.jinja"
try:
...
# keep track of total awards printed to determine which highlight color to use
award_num: int = 0
for tourn_filename in directory_list:
    ...
    for _, row in dfRankings.iterrows():
        ...
        award_num += 1
        teamList[div] += f"<p><span class=\"highlight" + str(award_num%2) + f"\">(Div {div}) Team number {team_num}, {team_name}</span></p>\n"
    ...
    for i in reversed(range(awardCounts["Robot Game"])):
        ...
        award_num += 1
        rg_html[div] += (
            "<p><span class=\"highlight" + str(award_num%2) + "\">With a score of "
            + str(score)
            + " points, the Division "
            + str(div)
            + " "
    ...
    for award in awards:
        ...
        for i in reversed(range(awardCounts[award])):
            ...
            try:
                ...
                award_num += 1
                thisText = (
                    "<p><span class=\"highlight" + str(award_num%2) + "\">The Division "
                    + str(div)
                    + " "
                    + ordinals[i]
                    + " place "
    ...
    for index, row in advancingDf[div - 1].iterrows():
        ...
        award_num += 1
        advancingHtml[div] += (
            "<p><span class=\"highlight" + str(award_num%2) + "\">(Div "
            + str(div)
            + ") Team number "
            + teamNum
            + ", "
...
out_text = template.render(
    ...
    bg_color_1 = "yellow" if dfMeta.loc[
        dfMeta["Key"] == "dual_emcee", "Value"
    ].values[0] else "transparent",
    bg_color_0 = "lightblue" if dfMeta.loc[
        dfMeta["Key"] == "dual_emcee", "Value"
    ].values[0] else "transparent",
    ...
    rd_this_these="This team" if awardCounts["Robot Design"] == 1 else "These teams",
